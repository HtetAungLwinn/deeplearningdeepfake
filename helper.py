...
# Custom dataset to override labels (0 = real, 1 = fake)
...
# Extract indices of real and fake samples
def get_indices(dataset):
    real_indices = [i for i, label in enumerate(dataset.targets) 
                    if label == dataset.class_to_idx['real']]
    fake_indices = [i for i, label in enumerate(dataset.targets) 
                    if label == dataset.class_to_idx['fake']]
    return real_indices, fake_indices


# Build dataset with controlled number of real/fake samples
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for images, labels in valid_loader:
                ...
                all_probs.extend(probs[:, 1].cpu().numpy().flatten())  # prob of class 1 (fake)
